...
app.config.update(
    ...
    MAIL_PASSWORD = params['gmail_psswrd']
)
mail = Mail(app)
...
def allpost():
    posts = Posts.query.filter_by().all()
    last = math.ceil(len(posts)/int(params['posts_page_post']))
    # [0:params['no_of_posts']]
    page = request.args.get('page')
    if (not str(page).isnumeric()):
        page = 1
    ...
    posts = posts[(page-1)*int(params['posts_page_post']):(page-1)*int(params['posts_page_post'])+int(params['posts_page_post'])]
    # Pagination Logic
    # first
    if (page==1):
        prev = "#"
        next = "/all-post?page="+ str(page+1)
    # last
    elif (page==last):
        prev = "/all-post?page="+ str(page-1)
        next = "#"
    # middle
    else:
        prev = "/all-post?page="+ str(page-1)
    ...
    return render_template("allpost.html", params=params, posts=posts, prev=prev, next=next)
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            if sno=='0':
                post = Posts(title=box_title, tagline=box_tline, slug=box_slug, content=box_content, img_file=box_img_file, date=date)
                db.session.add(post)
                db.session.commit()
            else:
                post = Posts.query.filter_by(sno=sno).first()
